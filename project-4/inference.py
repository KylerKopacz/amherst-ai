...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # Inference equation:
        #p{X is true} = P{Observed dist = true} * P{it was true in the past}
        ...
        if noisyDistance==None :
            for p in self.legalPositions:
                allPossible[p] = 0
            # self.getJailPositions() didn't even have a probability for that spot before
            # so we got to add it
            ...
            for p in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    # what we think is there is what we though before, multiplied by our new evidence
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # create a new beliefs system
        allPossible = util.Counter()

        # for every place that the ghost could be right now
        for oldPos in self.legalPositions:

            # get the position distribution of where the ghosts could be a step from now
            # below is the P(Ghost is at p at t + t | Ghost is at oldPos right now)
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))

            # for every one of those possible places that the ghost could be next
            for newPos, prob in newPosDist.items():
                if prob > 0:
                    # update the beliefs at the newPos to equal what we already thought about that
                    # state, plus the probability that the ghost is actually there (I THINK THAT'S 
                    # WHAT'S GOING ON, got lucky tbh)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        #number of particles & positions
        numParticles=self.numParticles
        numPositions= len(self.legalPositions)

        #create a list of unitalized particles
        particleList = [None]*numParticles

        #fill the particlelist evenly with positions
        i=0
    ...
    def observe(self, observation, gameState):
        ...
        # edge case 1
        if(noisyDistance== None):
            particleList= [self.getJailPosition()] * self.numParticles
        else:
            N= len(particleList)
            # intialize locals (wieghts)
            weights= util.Counter()
            # now we do steps 1 through 3.
            #print(beliefs)
            for i in range(0,N):
                # Step 2: Weight each state based on evidence, we can add the wieghts together because each wieght is independant
                weights[particleList[i]] += emissionModel[util.manhattanDistance(particleList[i],pacmanPosition)]
            if weights.totalCount()==0:
                particleList=self.initializeUniformly(gameState)
                # print("CORNERCASE---------------------------------------")
            else:
            #Step 3: Weighted-sample-with-replacement
                #print(weights)
                particleList= util.nSample(weights.values(),weights.keys(),N)
        self.particleList=particleList
        return particleList
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in beliefs:
            ...
            for newPos, prob in newPosDist.items():
                if prob > 0:
                    # find the odds that the ghost is at that place thing
                    allPossible[newPos] = allPossible[newPos] + beliefs[oldPos] * prob

        # now normalize that
        allPossible.normalize()
        self.particleList = util.nSample(allPossible.values(), allPossible.keys(), len(self.particleList))
    ...
    def getBeliefDistribution(self):
        ...
        #essentialy, we want to generate a counter
        #with keys of every position and values of the number of partilces "at" that position.
        #then we can just normalize it to set a distrobution
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for  i in range(0,N):
            #the wieght of every particle should be the multaplicative sum of the indovidual location's probabilities based on the emissionModel
            weightAtState = 1
            for ghostIndex in range(0,numGhosts):
                #Edge case 1- Account for potenial felon ghosts
                if noisyDistances[ghostIndex]== None:
                    particleList[i] = self.getParticleWithGhostInJail(particleList[i],ghostIndex)
                else: #skip ghosts that are in jail
                    weightAtState*=emissionModels[ghostIndex][util.manhattanDistance(particleList[i][ghostIndex],pacmanPosition)]
            #the belief for a state
            beliefs[particleList[i]]+= weightAtState

        # Edge Case 2: our weights are 0-- note still account for jail?
            ...
            for ghost in range(numGhosts):
                if noisyDistances[ghost]== None:
                    for i in range(len(particleList)):
                        particleList[i] = self.getParticleWithGhostInJail(particleList[i],ghost)
            self.particles=particleList
            return particleList
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                #distrbution of new positions for one ghost based on the old position
                newPosDist = getPositionDistributionForGhost(
                    setGhostPositions(gameState, list(oldParticle)), i, self.ghostAgents[i])

                #resample that particular location based on the distrobution
