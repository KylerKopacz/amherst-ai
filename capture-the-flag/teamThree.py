...
def createTeam(firstIndex, secondIndex, isRed,
               first = 'QPerceptAgent', second = 'OffensiveReflexAgent'):
  """
...
class QPerceptAgent(RAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        self.weights= {'bias':1.0,'score':1.0,'onDefense':1.0,'numInvaders':1.0,'closestEnemy':1.0, 'distanceToFood':1.0,'distanceToCapsule':1.0,'foodToDefend':1.0}
...
## Q Learning stuff!!!
    ...
    def getReward(self, gameState,nextState):
        ...
        if nextPos == [enemypos for enemypos in invaders]:
            #2  & #4 Kills an enemy (either a scared ghost or a pacman)+ 100 points
            if enemy.isPacman or bool(enemy.gameState.data.agentStates[enemy.index].scaredTimer):
                reward+=100
    ...
    def getFeatures(self, gameState, action):
        ...
        #============
        # Defensive features: food remaining
        features['foodToDefend']= len(myFoodList)
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    myState = successor.getAgentState(self.index)
    myPos = myState.getPosition()
    teammatePos = [observation.getAgentPosition(i) for i in range(observation.getNumAgents()) if i != self.index and i not in gameState.getRedTeamIndices()]
    ...
    if len(enemyDists) > 0 and myState.isPacman:
      ...
      if minDistFromEnemy <= 5:
        # we need to make a beeline for the nearest friendly ghost
        minDistanceToSide = self.getMazeDistance(myPos, teammatePos[0])
