...
def createTeam(firstIndex, secondIndex, isRed,
               first = 'DQPerceptAgent', second = 'OffensiveReflexAgent'):
  """
...
class OQAgent(RAgent):
    """
    A reflex agent that seeks food. The agent acts based on its extensive training (using q learning), and makes perceptions based on particle filtering. Note that this agent is NOT programmed to play offense
    """
    def registerInitialState(self, gameState):
        self.start = gameState.getAgentPosition(self.index)
        CaptureAgent.registerInitialState(self, gameState)
        ...
        self.explorationRate = 0.05#Epsilon
        self.learningRate = 0.2# Alpha
        self.discountRate =0.6
        self.weights= {'score':1.0,'successorScore':1.0,'onDefense':1.0,'numInvaders':1.0, 'distanceToFood':1.0,'retreat':1.0,'closestEnemy':1.0}

        # read from a file the wieghts
        try:
            with open('qlearndataO.txt', "r") as file:
...
## Q Learning stuff!!!
    ...
    def update(self, gameState, action):
        ...
        #print('Wieghts are:' + str(self.weights))
    ...
    def getFeatures(self, gameState, action):
        ...
        #=============
        # Offesnive Features: Food, Closest Exit, minDistFromEnemy
        ...
        if len(dists) > 0 and myState.isPacman:
          minDistFromEnemy = min(dists)
          ...
          if minDistFromEnemy <= 5:
            ...
            minDistanceToSide = min([self.distancer.getDistance(myPos, food) for food in myFoodList])
    ...
    def final(self,gameState):
        ...
        file=open('qlearndataO.txt','w')
        file.write(str(self.weights))

class DQPerceptAgent(RAgent):
    """
    ...
    def registerInitialState(self, gameState):
        ...
        self.weights= {'score':1.0,'successorScore':1.0,'onDefense':1.0,'numInvaders':1.0,'invaderDistance':1.0, 'enemyOneDistance':1.0,'enemyTwoDistance':1.0, 'distanceToFood':1.0,'retreat':1.0}

        # read from a file the wieghts
        try:
            with open('qlearndataD.txt', "r") as file:
                self.weights =eval(file.read())
        except IOError:
            return
    ...
    def observeState(self, gameState):
        ...
        for  i in range(0,N):
            ...
            for ghostIndex in range(0,numGhosts):
                weightAtState*= gameState.getDistanceProb(self.agent.distancer.getDistance(particleList[i][ghostIndex],pacmanPosition),noisyDistances[ghostIndex])
            #update beliefs
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if len(enemyDists) > 0 and myState.isPacman:
      ...
      if minDistFromEnemy <= 5:
        # we need to make a beeline for the nearest friendly food
        minDistanceToSide = min([self.getMazeDistance(myPos, food) for food in myFoodList])
        features['retreat'] = minDistanceToSide
